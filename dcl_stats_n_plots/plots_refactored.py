...
# Cell
class TwoDependentSamplesPlots(PlotHandler):
    ...
    def add_handler_specific_plots(self) -> Tuple[plt.Figure, plt.Axes]:
        ...
        for subject_id in self.data[subject_column_name].unique():
            y_group_a = self.data.loc[(self.data[subject_column_name] == subject_id) & (self.data[group_column_name] == self.configs.l_xlabel_order[0]), data_column_name].values[0]
            y_group_b = self.data.loc[(self.data[subject_column_name] == subject_id) & (self.data[group_column_name] == self.configs.l_xlabel_order[1]), data_column_name].values[0]
            ax.plot([0, 1], [y_group_a, y_group_b], color = self.configs.rm_linecolor, alpha = self.configs.rm_alpha, linewidth = self.configs.rm_linewidth, linestyle = self.configs.rm_linestyle)
        ax.set_xlim(-0.5, 1.5)
        return fig, ax
