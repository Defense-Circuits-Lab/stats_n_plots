...
from .plots_refactored import OneSamplePlots, MultipleIndependentSamplesPlots, MixedModelANOVAPlots, TwoDependentSamplesPlots
...
# Cell
class TwoDependentSamplesStats(StatisticalTest):

    @property
    def name_displayed_in_gui(self):
        return 'Comparison of values from two dependent samples (i.e. data with repeated measurements)'

    @property
    def plot_handler(self):
        return TwoDependentSamplesPlots


    def add_test_specific_information_to_df_infos(self) -> Dict:
        df_infos = self.lut['df_infos'].copy()
        df_infos['subject_column_name'] = self.df.columns[2]
        self.raise_error_if_not_all_subjects_have_repeated_measures(tmp_df_info = df_infos)
        return df_infos


    def raise_error_if_not_all_subjects_have_repeated_measures(self, tmp_df_info: Dict) -> None:
        group_column_name, subject_column_name = tmp_df_info['group_column_name'], tmp_df_info['subject_column_name']
        if len(tmp_df_info['all_group_ids']) != 2:
            raise ValueError(f'You can only use this test to compare the data of two sessions! Your current input data has {len(tmp_df_info["all_group_ids"])}.')
        id_group_a, id_group_b = tmp_df_info['all_group_ids']
        subject_ids_group_a = list(self.df.loc[self.df[group_column_name] == id_group_a, subject_column_name].unique())
        subject_ids_group_b = list(self.df.loc[self.df[group_column_name] == id_group_b, subject_column_name].unique())
        if len(subject_ids_group_a) == len(subject_ids_group_b):
            if all([True for elem in subject_ids_group_a if elem in subject_ids_group_b]):
                return None
            else:
                raise ValueError('Please check your input data again, as not all subjects have repeated measurements!')
        else:
            raise ValueError('Please check your input data again, as not all subjects have repeated measurements!')
